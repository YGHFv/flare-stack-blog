...
export const CodeBlock = memo(
  ({ code, language, highlightedHtml }: CodeBlockProps) => {
    const fallback = `<pre class="shiki font-mono text-sm leading-relaxed whitespace-pre text-(--atelier-btn-content) bg-transparent! p-0 m-0 border-0"><code>${code}</code></pre>`;
    ...
    return (
      <div className="relative group max-w-full my-6 not-prose">
        <div className="expressive-code relative rounded-xl border border-black/10 dark:border-white/10 bg-(--atelier-code-bg) overflow-hidden transition-colors shadow-sm">
          {/* Language Badge */}
          <div
            className={cn(
              ...
              "text-(--atelier-primary) bg-(--atelier-primary)/10 rounded-lg",
          ...
          <div
            className={cn("atelier-code-folding-wrapper custom-scrollbar")}
            style={
              needsFolding
          ...
          >
            ...
            {needsFolding && (
              <div
                className={cn(
                  "atelier-code-mask transition-opacity duration-500",
