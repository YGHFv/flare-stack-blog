...
export const AtelierCommentItem = memo(
  ({
    comment,
    onReply,
    onDelete,
    ...
    const renderedContent = useMemo(() => {
      if (comment.status === "deleted") {
        return (
          <p className="text-sm italic atelier-text-30 py-1">
    ...
    return (
      <div
        ...
        className={cn(
          ...
          isHighlighted &&
            "bg-(--atelier-btn-plain-bg-hover) -mx-4 px-4 rounded-xl",
          className,
        )}
      >
        {/* Avatar */}
        <div className="shrink-0 pt-0.5">
          <div className="w-9 h-9 rounded-full bg-black/5 dark:bg-white/10 overflow-hidden flex items-center justify-center transition">
            {comment.status === "deleted" ? (
              <span className="text-xs atelier-text-30">x</span>
            ) : comment.user?.image ? (
              <img
              ...
              <span className="text-sm font-medium atelier-text-50">
        ...
        <div className="flex-1 min-w-0 space-y-1.5">
          <div className="flex flex-wrap items-baseline justify-between gap-x-2 gap-y-1">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium atelier-text-75">
                {comment.status === "deleted"
                  ? m.comments_item_deleted_author()
                  : comment.user?.name || m.comments_item_anonymous()}
              </span>
              {isBlogger && comment.status !== "deleted" && (
                <span className="text-[10px] font-medium text-(--atelier-primary) border border-(--atelier-primary)/30 px-1.5 py-0.5 rounded-md leading-none">
                  {m.comments_item_blogger()}
                </span>
              )}

              {isReply && replyToName && (
                <span className="text-xs atelier-text-30">
          ...
          {comment.status !== "deleted" && (
            <div className="flex items-center gap-4 pt-1">
              <button
                ...
                className="text-xs atelier-text-30 hover:text-(--atelier-primary) transition-colors font-medium"
              >
              ...
              {(isAuthor || isAdmin) && (
                <button
                  ...
                  className="text-xs atelier-text-30 hover:text-red-500 transition-colors font-medium"
                >
...
AtelierCommentItem.displayName = "AtelierCommentItem";
