...
import { AtelierCommentList } from "./comment-list";
import AtelierConfirmationModal from "./confirmation-modal";

const routeApi = getRouteApi("/_public/post/$slug");

interface AtelierCommentSectionProps {
  postId: number;
}

export function AtelierCommentSection({ postId }: AtelierCommentSectionProps) {
  ...
  if (isLoading || !data) {
    return <AtelierCommentSectionSkeleton />;
  }
    ...
    <div className="space-y-6">
      ...
      {session ? (
        ...
        <div className="py-10 flex flex-col items-center justify-center gap-3 text-center">
          ...
          <Link to="/login">
            <button className="atelier-btn-primary h-9 px-5 text-sm rounded-lg gap-2">
              <LogIn size={14} />
              {m.comments_login()}
            </button>
          </Link>
      ...
      {hasNextPage && (
        <div className="flex justify-center pt-4">
          <button
            ...
            className="atelier-btn-regular h-10 px-6 text-sm rounded-lg disabled:opacity-50"
          >
...
function AtelierCommentSectionSkeleton() {
  return (
    <div className="space-y-6">
