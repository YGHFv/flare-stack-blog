...
export const AtelierCommentList = ({
  rootComments,
  postId,
  onReply,
  onDelete,
            ...
            <div
              className={`h-px transition-all duration-300 ${
                isExpanded
                  ...
                  : "w-6 bg-black/10 dark:bg-white/10 group-hover:w-10 group-hover:bg-(--atelier-primary)/50"
              }`}
            />
            <span className="text-xs atelier-text-50 group-hover:text-(--atelier-primary) transition-colors">
              {isExpanded
            ...
            <div className="mt-2 space-y-0 pl-4">
              ...
              {hasNextPage && (
                <button
                  ...
                  className="text-xs atelier-text-50 hover:text-(--atelier-primary) transition-colors font-medium py-2 disabled:opacity-50"
                >
                  {isFetchingNextPage
                    ? m.comments_loading()
...
function LoginToReplyPrompt({
  ...
  return (
    <div className="flex items-center gap-4 py-3 px-4 rounded-(--atelier-radius-large) bg-(--atelier-input-bg)">
      <span className="text-sm atelier-text-50 flex-1">
        {m.comments_list_login_to_reply({ userName: userName })}
      </span>
      <Link to="/login">
        <button className="atelier-btn-primary h-8 px-4 text-sm rounded-lg">
          {m.comments_login()}
        </button>
      </Link>
      {onCancel && (
        <button
          onClick={onCancel}
          className="text-sm atelier-text-50 hover:atelier-text-75 transition-colors"
        >
