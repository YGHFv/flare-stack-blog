...
interface AtelierInsertModalProps {
  type: ModalType;
  initialUrl?: string;
  onClose: () => void;
...
const AtelierInsertModalInternal: React.FC<AtelierInsertModalProps> = ({
  type,
  initialUrl = "",
  onClose,
  onSubmit,
  ...
  return createPortal(
    ...
    >
      ...
      <div
        className={`
          relative w-full max-w-md atelier-card-base p-0
      ...
      >
        ...
        <div className="px-6 pt-6 pb-4 flex items-center justify-between">
          <h2 className="text-lg font-bold atelier-text-90">
            {activeType === "LINK"
          ...
          <button
            onClick={onClose}
            className="p-1 -mr-1 atelier-text-30 hover:atelier-text-75 transition-colors"
          >
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-4 flex flex-col gap-4">
          <div className="space-y-2">
            <label className="text-sm font-medium atelier-text-75 ml-1">
              {activeType === "IMAGE"
            ...
            <input
              ...
              placeholder="https://..."
              className="w-full px-4 py-2.5 rounded-xl border border-(--atelier-input-border) bg-(--atelier-input-bg) focus:outline-none focus:border-(--atelier-primary)/50 transition-all atelier-text-90 text-sm"
            />
          </div>

          <p className="text-xs atelier-text-30 px-1">
            {activeType === "LINK"
        ...
        <div className="px-6 pb-6 flex justify-end gap-3 mt-2">
          <button
            onClick={onClose}
            type="button"
            className="h-10 px-4 text-sm atelier-text-50 hover:atelier-text-75 transition-colors font-medium"
          >
          ...
          <button
            ...
            className="atelier-btn-primary h-10 px-6 text-sm rounded-xl font-bold disabled:opacity-40 disabled:cursor-not-allowed"
          >
...
export const AtelierInsertModal: React.FC<AtelierInsertModalProps> = (
  props,
) => {
  return (
    <ClientOnly>
      <AtelierInsertModalInternal {...props} />
    </ClientOnly>
  );
};
