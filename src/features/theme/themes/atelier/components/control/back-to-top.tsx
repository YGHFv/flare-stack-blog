...
export function BackToTop() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Same logic as original Atelier: show after banner height
      // The current Atelier banner logic sets banner to ~35vh or 65vh, let's use 35vh as base threshold
    ...
    <div className="hidden lg:block absolute right-0 top-0 w-15 h-15 pointer-events-none">
      ...
      >
        <button
          ...
          className="flex items-center justify-center w-15 h-15 atelier-card-base hover:bg-(--atelier-btn-plain-bg-hover) active:bg-(--atelier-btn-plain-bg-active) text-(--atelier-primary) text-2xl font-bold transition-all active:scale-90 shadow-md"
        >
