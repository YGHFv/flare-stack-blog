      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-sm font-medium atelier-text-50">
          {isEmailConfigured
            ...
            : m.login_only_third_party_atelier()}
        </p>
      </div>
      ...
      <div className="flex flex-col gap-6">
        ...
        {isEmailConfigured && (
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5 focus-within:text-(--atelier-primary) transition-colors text-(--atelier-text-50)">
              <div className="flex justify-between items-center ml-1">
                ...
                <Link
                  to="/forgot-password"
                  tabIndex={-1}
                  className="text-xs font-medium hover:text-(--atelier-primary) transition-colors"
                >
                  {m.login_forgot_password_atelier()}
                </Link>
              </div>
              <input
                id="login-password"
            ...
            <button
              type="submit"
              disabled={isFormDisabled}
              className="mt-2 w-full py-3.5 rounded-xl atelier-btn-primary font-bold text-sm tracking-wide active:scale-[0.98] transition-all gap-2"
            >
              {loginStep === "VERIFYING" ? (
                <>
        ...
        {isEmailConfigured && (
          <div className="relative flex items-center py-2">
            ...
            <span className="mx-4 text-xs font-medium atelier-text-30">
              {m.login_or()}
            </span>
            <div className="flex-1 border-t border-border/30"></div>
          </div>
        ...
        <button
          ...
          className={`group w-full py-3.5 rounded-xl flex gap-3 transition-all font-bold text-sm active:scale-[0.98] disabled:opacity-50 disabled:active:scale-100 ${
            !isEmailConfigured ? "atelier-btn-primary" : "atelier-btn-regular"
          }`}
        >
          {socialIsLoading ? (
