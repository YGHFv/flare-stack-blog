      ...
      <div className="flex items-center">
        ...
        <div className="flex flex-row flex-nowrap items-center gap-x-1.5">
          {post.tags && post.tags.length > 0 ? (
            post.tags.map((tag, i) => (
              <span key={tag.name} className="flex items-center">
                ...
                <Link
                  ...
                  className="transition atelier-text-50 text-sm font-medium hover:text-(--atelier-primary) whitespace-nowrap"
                >
                  {tag.name}
                </Link>
              </span>
            ...
            <span className="transition atelier-text-50 text-sm font-medium">
              {m.post_no_tags()}
            </span>
          )}
        </div>
