...
export default function TableOfContents({
  ...
  // Section-based active heading detection (matches original Atelier logic)
  const computeActiveHeadings = useCallback(() => {
    ...
    for (let i = 0; i < headers.length; i++) {
      ...
      if (i < headers.length - 1) {
        ...
        const content = heading.closest(".atelier-custom-md");
        if (content) {
  ...
  return (
    ...
    >
      <div
        ref={tocRootRef}
        className="relative toc-root overflow-y-scroll overflow-x-hidden atelier-toc-scrollbar h-[calc(100vh-20rem)]"
        style={{
              ...
              return (
                <a
                  ...
                  className={cn(
                    ...
                    "hover:bg-(--atelier-toc-btn-hover) active:bg-(--atelier-toc-btn-active)",
                  )}
                >
                  <div
                    className={cn(
                      ...
                      {
                        "bg-[oklch(0.89_0.050_var(--atelier-hue))] dark:bg-(--atelier-btn-regular-bg) text-(--atelier-btn-content)":
                          isH1,
                        "ml-4": isH2,
                        "ml-8": isH3,
                      },
                  ...
                  >
                    {isH1 && h1Count++}
                    {isH2 && (
                      <div className="transition w-2 h-2 rounded-[0.1875rem] bg-[oklch(0.89_0.050_var(--atelier-hue))] dark:bg-(--atelier-btn-regular-bg)"></div>
                    )}
                    {isH3 && (
                  ...
                  <div
                    className={cn("transition text-sm", {
                      "atelier-text-50": isH1 || isH2,
                      "atelier-text-30": isH3,
                    })}
                  >
                    {text}
                  </div>
          ...
          {headers.length > 0 && (
            <div
              className={cn(
                ...
                "bg-(--atelier-toc-btn-hover) border-(--atelier-toc-btn-hover) group-hover:bg-transparent group-hover:border-(--atelier-toc-btn-active)",
              )}
              style={{
