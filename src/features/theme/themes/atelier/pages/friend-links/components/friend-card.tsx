...
export function FriendCard({ link, className, style }: FriendCardProps) {
  ...
  return (
    <a
      ...
      className={cn(
        "atelier-card-base block relative p-4 transition-all duration-300",
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1 flex flex-col justify-center">
          <h3 className="text-lg font-bold atelier-text-90 truncate transition-colors duration-300 group-hover:text-(--atelier-primary) flex items-center gap-1.5">
            {link.siteName}
            <ExternalLink className="w-4 h-4 opacity-0 -translate-x-2 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0 group-hover:text-(--atelier-primary)" />
          </h3>
          <p
            className="text-sm atelier-text-50 mt-1 line-clamp-2 leading-relaxed"
            title={description}
          >
            {description}
          </p>
